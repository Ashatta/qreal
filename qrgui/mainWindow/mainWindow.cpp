...
{
	mUi->setupUi(this);
	mUi->paletteTree->initModels(&models());
	mController = new Controller(models().repoControlApi());
...
void MainWindow::createDiagram(const QString &idString)
{
	...
	models().repoControlApi().addLogEntries(created, QList<qReal::migration::LogEntry *>());
...
{
	PropertiesDialog propDialog(mFacade.editorManager(), models().mutableLogicalRepoApi(), id);
...
{
	openShapeEditor(id, shape, &mFacade.editorManager(), false);
}
...
void MainWindow::deleteElementType(const Id &deletedElement, bool isRootDiagram)
{
	...
	mFacade.editorManager().deleteElement(deletedElement);
...
{
	ChooseTypeDialog chooseTypeDialog(editor, mFacade.editorManager(), this);
